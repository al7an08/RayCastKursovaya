...
void Player::update(const std::array<std::array<Cell, MAP_HEIGHT>, MAP_WIDTH>& i_map, const sf::RenderWindow& i_window)
{
	...
	for (unsigned short a = 0; a < SCREEN_WIDTH; a++)
	{
		...
		//Мы продолжаем бросать лучи, пока он не достигнет ограничения по дистанции рендера
		while (RENDER_DISTANCE >= ray_length)
		{
			//На случай если луч попадет в угол
			bool corner_collision = 0;

			//Увеличиваем самый коротокий из лучей
			...
			{
				//Если лучи одинаковые по длине, что значит, что попали в угол, увеличваем оба луча
				corner_collision = 1;
